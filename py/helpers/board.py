...
class Board:
    def __init__(self, conffile):
        try:
            ...
            self.drop_stones = record["drop stones"] if "drop stones" in record.keys() else [round(self.reference_point[i] - 2 * self.x_shift[i] + 2 * self.y_shift[i], 1) for i in range(3)]
            self.max_height = max(
                record["board"][0][2],
                record["board"][1][2],
                record["board"][2][2],
